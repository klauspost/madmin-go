...
//go:generate msgp -file=$GOFILE -unexported -d "timezone utc" -d "clearomitted" -d "tag json"
...
//msgp:ignore ServiceTraceInfo ServiceTraceOpts
...
func (adm AdminClient) ServiceTrace(ctx context.Context, opts ServiceTraceOpts) <-chan ServiceTraceInfo {
	...
	go func(traceInfoCh chan<- ServiceTraceInfo) {
		...
		for {
			...
			var decodeOne func(m *TraceInfo) error
			switch resp.Header.Get("Content-Type") {
			case "application/vnd.msgpack":
				dec := msgp.NewReader(resp.Body)
				decodeOne = func(m *TraceInfo) error {
